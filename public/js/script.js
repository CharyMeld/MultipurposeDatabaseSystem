...
$(document).ready(function(){
	...
	$('li[data-critical=1] a').click(function(event){
		...
		if (bulkAction) {
			// get the list of ids and send to to the server
			initBulkAction($(this));
			return ;
		}else{
...
//add toggle event
	...
	$('.paged-item').click(function(e){
		...
		var hasRightValue = start!=''&& start!=undefined && len!='' & len!=undefined;
		if (isEnabled && hasRightValue) {
	...
	$('#page_size').change(function(event) {
		...
		var start =0;
		var len = $(this).val();
		if (len && len.toLowerCase().trim()=='all') {
			len=-1;
		}
		path =replaceOrAdd(path,'p_start',start);
		path = replaceOrAdd(path,'p_len',len);
...
function addResultMoveOperation() {
	$('#move_result_btn').click(function(event) {
		// get the ids of the selected result and the value selected for session and level,
		// then move as necessary
		var boxes = $('.table-check > :checkbox:checked');
		var items = [];
		boxes.each(function(index, el) {
			items.push($(el).attr('data-item'));
		});
		var session = $('#move_session').val();
		var level = $('#move_level').val();
		if (!session) {
			showNotification(false,"kindly select session");
			return;
		}
		var link = $(this).attr('link');
		var data = {results:items,session:session,level:level};
		data = 'data='+JSON.stringify(data);
		if (confirm("are you sure you want to move item?")) {
			sendAjax(null,link,data,'post',reportAndRefresh);
		}
	});
}
...
function showNotification(status,data){//a boolean value for success or failure
	...
	console.log(notification.children('.message'));
	if (status) {
	...
	// var messageBox = $('#notification_message');
	// console.log(messageBox);
	notification.children('.message').text(data);
...
function bindDropDown(className){
	$('.'+className).change(function(event) {
		...
		var data='';
		var target =$('#'+child);
		url = $('#baseurl').val()+'ajaxData/'+path+'/'+val;
...
function addBulkOperation() {
	var checkboxes='.auto-table .table-check,.auto-table .table-check-header, #session_move_container';
	$('.auto-row').dblclick(function(event) {
	...
	$('.header-check').change(function(event) {
		var state = $(this).is(':checked');
		$('.table-check > :checkbox').prop('checked',state);
		// console.log(state);
		// $(".table-check").each(function(index, el) {
		// 	console.log(el);
		// 	el.checked=state;
		// });
	});
...
function initBulkAction(target){
	var boxes = $('.table-check > :checkbox:checked');
	var items = [];
	boxes.each(function(index, el) {
		items.push($(el).attr('data-item'));
	...
	var data = 'ids='+JSON.stringify(items);
